...
export async function startAgentSession(payload) {
  ...
  if (process.env.GROQ_API_KEY && process.env.LLM_API_URL) {
  ...
  return {
    ...
    transcript: {
      ...
      rawResponse: 'Generated by local fallback because GROQ_API_KEY or LLM_API_URL is not configured.'
    }
  };
}
...
export async function answerAgentQuestion(payload) {
  ...
  return {
    ...
    transcript: {
      ...
      rawResponse: 'Integrated by local fallback because GROQ_API_KEY or LLM_API_URL is not configured.'
    }
  };
}
...
async function refineProjectWithApi(payload) {
  ...
  if (!model) {
    throw new Error('LLM_MODEL is required when GROQ_API_KEY and LLM_API_URL are configured.');
  }
...
async function callGemini({ systemPrompt, payload, model, temperature }) {
  ...
  const response = await fetch(endpoint, {
    ...
    headers: {
      ...
      'x-goog-api-key': process.env.GROQ_API_KEY
    },
    body: JSON.stringify({
      systemInstruction: {
